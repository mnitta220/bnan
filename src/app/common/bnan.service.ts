...
export class BnanService {
  ...
  docViewPage: DocViewPage | null = null;
  retryCount = 0;
  ...
  async getSetting() {
    ...
    try {
      let ret = await Preferences.get({ key: SETTING_KEY });
      value = ret.value;
    } catch (e) {}

    if (!value) {
      try {
        value = window.localStorage.getItem(SETTING_KEY);
        if (value) {
          // localStorageからPreferencesへ移行
    ...
    if (!value) {
      ...
      this.setting = JSON.parse(value);

      if (
        this.setting &&
        this.setting.curDoc != null &&
        this.setting.curDoc.id != -1
      ) {
      ...
      if (this.setting?.version && this.setting.version < '1.11') {
  ...
  async getCurText() {
    ...
    this.curText = '';
    if (this._idb && this.setting?.curDoc) {
      try {
  ...
  async setCurrentDoc(id: number) {
    ...
    try {
      if (this.setting?.curDoc) this.setting.curDoc = null;

      if (this.setting && this._idb && this.wman) {
        await this._idb.docs
          .where('id')
          .equals(id)
          .first((d) => {
            if (d && this.setting && this.wman) {
        ...
        if (this.setting.curDoc) {
          this.wman.setDoc(
            this.setting.curDoc.id ?? 0,
  ...
  async newDoc(title: string, text: string, mode: number, fontSize: number) {
    ...
    try {
      ...
      if (this.setting && this.wman) {
        ...
        for (let l of lines) {
          let con = new Contents(doc.id ?? 0, 0, seq, l);
  ...
  async updateDoc(
    ...
    try {
      if (this.setting?.curDoc && this._idb && this.wman) {
        ...
        if (
          this.setting.curDoc.current > -1 &&
          lines[this.setting.curDoc.current].charAt(0) != '#'
        ) {
          this.setting.curDoc.current = -1;
        }
        ...
        for (let l of lines) {
          let con = new Contents(this.setting.curDoc.id ?? 0, newVer, seq, l);
          await this._idb.contents.add(con);
          ...
          switch (step) {
            ...
            case 1:
              if (con.type == 0) {
                ...
                if (type == 0) {
                  step = 2;
                } else if (con.type && con.type <= type) {
                  step = 2;
                }
                isOut = true;
              }
  ...
  async deleteDoc() {
    ...
    try {
      ...
      if (this.setting && this.setting.curDoc) {
        if (this._idb == null) {
          this._idb = new AppDatabase();
        }
  ...
  async updateCurrent(current: number) {
    ...
    try {
      ...
      if (this.setting && this.setting.curDoc && this.wman) {
  ...
  setFontSize() {
    ...
    if (this.setting) {
      ...
      if (this.selectedIndex == Define.PG_VIEW && this.docViewPage) {
  ...
  async zoomText(val: number) {
    ...
    if (this.setting) {
      ...
      if (
        (val == 1 && this.setting.zoom >= Define.ZOOM_MAX) ||
